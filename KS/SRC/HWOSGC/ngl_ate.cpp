...
u32 fixu32( u32 fuckedupendian )
{
	return (( fuckedupendian & 0x000000FF) << 24) +
	       (( fuckedupendian & 0x0000FF00) <<  8) +
	       (( fuckedupendian & 0x00FF0000) >>  8) +
	       (( fuckedupendian & 0xFF000000) >> 24);
}

u16 fixu16( u16 fuckedupendian )
{
	return (( fuckedupendian & 0x00FF) <<  8) +
	       (( fuckedupendian & 0xFF00) >>  8);
}

void FixupATEForBogusEndianMachines( char *atefile )
{
	ATEFileHeader *afh=(ATEFileHeader *) atefile;
	afh->magic   =fixu32(afh->magic   );
	afh->vermajor=fixu16(afh->vermajor);
	afh->verminor=fixu16(afh->verminor);
	afh->items   =fixu32(afh->items   );
	if ( afh->items )
	{
		for ( ATE_UINT32 i=0; i<afh->items; i++ )
		{
			...
			//atestring     name;
			afe->hoff    =fixu32(afe->hoff    );
			afe->ioff    =fixu32(afe->ioff    );
			afe->poff    =fixu32(afe->poff    );
		}
	}
...
bool ATENameMatch(const atestring &find,const atestring &entry )
{
	...
	assert(0); // Implement strnicmp if other projects need it.
	return 0;
#else
