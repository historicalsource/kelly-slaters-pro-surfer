...
//  int offset;
  ...
  if (gas.source_list[source].flag.stereo && ((load_size % 2) != 0))
  {
...
int _gas_update_reclaim_voices()
{
  ...
  for (v=0; v<gas.inst_list_count; ++v)
  {
        ...
        (gas.inst_list[v].flag.loop == 0))
    {
      ...
      {
#ifdef DEBUG_OUTPUT
printf("Reclaiming inst 0x%x %s\n", gas.inst_list[v].instance_id, gas.source_list[gas.inst_list[v].source].short_filename);
#endif
...
gas.voice_used[v] = -1;
    _gas_update_recycle_voice(this->flag.corel, this->flag.voicel);
    this->flag.corel = 0;
...
gas.voice_used[v] = -1;
      _gas_update_recycle_voice(this->flag.corer, this->flag.voicer);
      this->flag.corer = 0;
...
//
void _gas_update_recycle_voice(int core, int voice)
{
  // Assumed semaphore locks: system_state_sema
  RECORDRA;
#ifdef DEBUG_OUTPUT
