...
// inst = gas_rpc (GAS_RPC_ADD_INSTANCE, NULL, gSrcBlues, 0, 0, 0 );
// gas_rpc (GAS_RPC_PLAY_INSTANCE, NULL, inst, 0, 0, 0 );
// gas_rpc (GAS_RPC_PAUSE_INSTANCE, NULL, inst, 0, 0, 0 );
// gas_rpc (GAS_RPC_UNPAUSE_INSTANCE, NULL, inst, 0, 0, 0 );
// gas_rpc (GAS_RPC_STOP_INSTANCE, NULL, inst, 0, 0, 0 );

#define DATA_SIZE_COMMAND GAS_COMMAND_LIST_SIZE
#define DATA_SIZE_NORMAL  sizeof(GasRpcArgs)

GasStatus g_iop_status;
...
/*** gas rpc function calls (to the gas_iop module) ***/
int gas_rpc_init (void);
int gas_rpc (int command, char *strarg, int intarg0, int intarg1, int intarg2, int intarg3 );
void set_3d_volume(int inst, int angle, unsigned short vol, short *volume_left, short *volume_right);

/*** gas_volume ***/ 
// sets the volume for the sound (surround channel ignored for stereo sounds)
inline int gas_volume(int instance_id, unsigned short volume_left, unsigned short volume_right)
{
  int inst = instance_id & GAS_INSTANCE_ID_MASK;

  if (instance_id == -1)
  {
    nglPrintf("Error.  Invalid instance (0x%x)\n", instance_id); 
    return 0;
  }
  command_list[inst].instance_id = instance_id;
  command_list[inst].set_volume = 1;
  command_list[inst].volume_left = volume_left;
  command_list[inst].volume_right = volume_right;
//  gas_rpc (GAS_RPC_INSTANCE_VOLUME, NULL, instance_id, volume_left, volume_right, 0 );
  return 1;
}
...
/*  if (os_developer_options::inst()->is_flagged(os_developer_options::FLAG_NO_AUDIO))
  ...
  else
  {
    ...
    debug_dir += "c:\\ks\\src\\hwosps2\\gas\\";
...
{/*
  if (!os_developer_options::inst()->is_flagged(os_developer_options::FLAG_NO_AUDIO))
  {
    ...
    int ps2_volume = (int)(new_volume * SCE_VOLUME_MAX);
    gas_rpc(GAS_RPC_SET_MASTER_VOLUME, NULL, ps2_volume, 0, 0, 0);
  }*/
}
...
// replacing the integer divide with a shift, since int devides are so slow on the ps2.
...
// the ps2 exhibits audible 'pops' when the volume is updated too often.
...
/*-------------------------------------------------------------------------------------------------------
  ...
  Ghastly Audio System Remote Procedure Call (GAS RPC) implementation
...
static int last_command = 0;
  ...
  while (sceSifCheckStatRpc((sceSifRpcData *)&cd) == 1)
  {
//    nglPrintf("Waiting on RPC call\n");
  }
  if (last_command != 0)
  {
    memcpy(&g_iop_status, sbuf, sizeof(GasStatus));
    if (last_command == GAS_RPC_COMMAND_LIST)
      memset(&command_list, 0, GAS_COMMAND_LIST_SIZE);
  }
  ...
  if (command != GAS_RPC_COMMAND_LIST)
  {
    ...
    if (command & GAS_RPC_NON_BLOCKING_MASK)
      mode = SIF_RPCM_NOWAIT;
    else
      mode = 0;
    ret = sceSifCallRpc (&cd, command, mode, (void *) sbuf, arg_size,
      (void *) sbuf, sizeof(GasStatus), NULL, NULL);
  }
  else
  {
  ...
  else
  {
    memcpy(&g_iop_status, sbuf, sizeof(GasStatus));
    last_command = 0;
    return g_iop_status.rpc_retval1;
  }*/
  return 0;
}
...
short old_left = SCE_VOLUME_MAX / 2;
short old_right = SCE_VOLUME_MAX / 2;
...
//  gas_rpc (GAS_RPC_INSTANCE_VOLUME, NULL, inst, new_left & 0x7fff, new_right & 0x7fff, 0 );
  gas_volume(inst, *volume_left, *volume_right);
