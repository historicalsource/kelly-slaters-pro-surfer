...
static inline void fast_matrix4x4_mul(matrix4x4 &res, const matrix4x4& a, const matrix4x4& b)
{
  asm("lqc2    vf1,0x0(%2)  \n"
      ...
      "vmulax.xyzw	ACC,   vf1,vf5 \n"
	    ...
	    "sqc2    vf12,0x30(%0) \n"
	:
  : "r" ((sceVu0FMATRIX)&res.x.x), "r" ((sceVu0FMATRIX)&a.x.x), "r" ((sceVu0FMATRIX)&b.x.x)
  );
}

static inline matrix4x4 operator*( const matrix4x4& a, const matrix4x4& b )
//static inline matrix4x4 ps2_mul( const matrix4x4& a, const matrix4x4& b )
{
  matrix4x4 local;

  // Keep this code in sync with the function 'static inline void fast_matrix4x4_mul(matrix4x4 &res, const matrix4x4& a, const matrix4x4& b)'
  asm("lqc2    vf1,0x0(%2)  \n"
	    "lqc2    vf2,0x10(%2) \n"
	    "lqc2    vf3,0x20(%2) \n"
	    "lqc2    vf4,0x30(%2) \n"
      ...
      "vmulax.xyzw	ACC,   vf1,vf5 \n"
	    ...
	    "sqc2    vf12,0x30(%0) \n"
	:
  : "r" ((sceVu0FMATRIX)&local.x.x), "r" ((sceVu0FMATRIX)&a.x.x), "r" ((sceVu0FMATRIX)&b.x.x)
  );
